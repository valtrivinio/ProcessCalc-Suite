...
// Expanded pipe database (Schedule 40 & 80)
...
// Re-export the underlying calculations with the names the UI expects
export function calculateReynolds(rho: number, v: number, D: number, mu_cP: number) {
  return reynolds(rho, v, D, mu_cP);
}

export function calculateFrictionFactor(eps: number, D: number, Re: number) {
  return colebrookFriction(eps, D, Re);
}

// Darcy-Weisbach pressure drop (with optional minor losses)
...
// Original sizing function (kept for compatibility, but UI doesn't use it yet)
